...
def service(depth,overwrite):
    ...
    if overwrite:
        expand_data(depth, PATH_여성안심지킴이집_AFTER, PATH_여성안심지킴이집_AFTER, 'euc-kr')
        expand_data(depth, PATH_여성안심택배함_AFTER, PATH_여성안심택배함_AFTER, 'euc-kr')
        expand_data(depth, PATH_아동안전지킴이시설물_AFTER, PATH_아동안전지킴이시설물_AFTER, 'euc-kr')
        expand_data(depth, PATH_편의점_AFTER, PATH_편의점_AFTER, 'euc-kr')

        expand_data(depth, PATH_경찰서_AFTER, PATH_경찰서_AFTER, 'utf-8')
        expand_data(depth, PATH_치안센터_AFTER, PATH_치안센터_AFTER, 'utf-8')
        expand_data(depth, PATH_파출소지구대_AFTER, PATH_파출소지구대_AFTER, 'utf-8')
